...
const data = Object.keys(icons).sort().map((name) => ({
  ...
  inner: icons[name].elements.map(([tag, attrs]) => `<${tag} ${attrString(attrs)}/>`).join(''),
}));
...
<style>
  ...
  @media (prefers-color-scheme: dark) {
    :root {
      --paper: #161412; --ink: #e9e6e1; --muted: #98918a;
      --line: #2b2823; --line-strong: #453f38; --cell: #1d1a17;
      --accent: #82a7f2; --accent-ink: #10131c;
    }
  }
  * { box-sizing: border-box; margin: 0; }
  ...
  h1 { font-family: ui-monospace, Menlo, monospace; font-size: 21px; font-weight: 600; letter-spacing: -0.02em; }
  h1 .count { color: var(--muted); font-weight: 400; }
  .sub { color: var(--muted); margin-top: 8px; max-width: 56ch; }
  .sub a { color: inherit; }

  .controls { display: flex; flex-wrap: wrap; gap: 12px; align-items: center; margin-top: 32px; }
  .search {
  ...
  .card .hint {
    font: 11px ui-monospace, Menlo, monospace; color: var(--muted); display: block; margin-top: 2px;
  }
  .card .svgbtn {
  ...
  @media (prefers-reduced-motion: no-preference) {
    .card, .seg button, .cats button, .svgbtn { transition: border-color 150ms ease-out, color 150ms ease-out, background-color 150ms ease-out; }
...
<body>
<header>
  <h1>agenticons <span class="count" id="count"></span></h1>
...
for (const c of DATA.categories) {
  ...
  section.innerHTML = '<h2 class="cat-title">' + c + '</h2>';
  const grid = document.createElement('div');
  grid.className = 'grid';
  for (const icon of DATA.icons.filter((i) => i.category === c)) {
...
function apply() {
  ...
  count.textContent = '· ' + visible + (visible === 1 ? ' symbol' : ' symbols');
...
document.getElementById('size-seg').addEventListener('click', (e) => {
  ...
  document.body.style.setProperty('--icon-size', b.dataset.size + 'px');
});
...
document.getElementById('stroke-seg').addEventListener('click', (e) => {
  ...
  for (const btn of b.parentElement.children) btn.setAttribute('aria-pressed', btn === b);
  for (const svg of main.querySelectorAll('svg')) svg.setAttribute('stroke-width', stroke);
});
...
console.log(`gallery.html written (${data.length} icons)`);
